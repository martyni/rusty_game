...
class Game(object):
   ...
   def load_levels(self):
      ...
      for level in levels:
         ...
         self.levels[level.replace('.lvl','')] = Background(level, level_string=level_string, screen=self.screen, verbose=False)
   ...
   def loop(self):
      while True:
        ...
        self.width, self.height = self.controls.get_events(events=self.events, screen=self.screen, width=self.width, height=self.height)
